    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-darker px-4">
      <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="w-full max-w-sm">
        ...
        <div className="bg-brand-dark border border-brand-lime/10 rounded-2xl p-7 shadow-2xl">
          <h1 className="text-lg font-bold text-[#f0f4c0] mb-1">Broker admin</h1>
          <p className="text-xs text-brand-muted mb-6">Sign in to manage your clients</p>
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-[10px] font-semibold uppercase tracking-wider text-brand-muted mb-1.5">Email</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="broker@jrwfinance.com.au"
          ...
          <button onClick={onDemo} className="w-full py-2.5 rounded-lg text-xs font-medium border border-brand-lime/15 text-brand-muted hover:border-brand-lime/30 hover:text-brand-lime transition-all">Preview demo admin</button>
        </div>
      </motion.div>
    </div>
  )
